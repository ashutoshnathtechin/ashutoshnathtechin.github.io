      ...
      <div className="flex flex-col gap-10">
        {projects.map((project, idx) => (
          <Reveal key={idx} direction={idx % 2 === 0 ? 'left' : 'right'} delay={idx * 150}>
            <div className="group relative bg-white/50 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 p-8 md:p-10 rounded-3xl flex flex-col lg:flex-row gap-10 items-start transition-all duration-500 hover:shadow-[0_30px_60px_rgba(45,212,191,0.15)] hover:-translate-y-2 overflow-hidden">
              ...
              <div className="relative z-10 w-full lg:w-1/3 flex flex-col gap-5">
                <h3 className="text-3xl font-bold text-slate-900 dark:text-white group-hover:text-teal-500 transition-colors">{project.title}</h3>
                <div className="flex flex-wrap gap-2">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700">
                      {tag}
                    </span>
                  ))}
                </div>
                <div className="flex flex-wrap gap-4 mt-4">
                  <a href={project.link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center flex-1 gap-2 text-sm font-bold text-white bg-teal-600 hover:bg-teal-500 px-5 py-3 rounded-xl transition-all shadow-lg shadow-teal-500/30">
                    Live Demo
                  </a>
                  <a href={project.source} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-200 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 px-5 py-3 rounded-xl transition-all">
              ...
              <div className="relative z-10 w-full lg:w-2/3">
                <ul className="list-disc list-outside ml-6 text-slate-600 dark:text-slate-400 space-y-4 marker:text-teal-500 text-lg">
